...
write.csv2(res_merged, file = '/data/cephfs-2/unmirrored/projects/liposarcoma-wgs/20241004_CellCharter_Xenium/MDM2/20240704region_mergedDDLSvsWDLS.csv'
           , row.names = F)

res_dt_ints <- resall_dt_int[xG %in% c(res_merged$xG, 'MDM2', 'CDK4')]

pdf(file = paste0('/data/cephfs-2/unmirrored/projects/liposarcoma-wgs/20241004_CellCharter_Xenium/MDM2/20240704region_mergedDDLSvsWDLS.pdf'))
print(ggplot(res_dt_ints, aes(x=log2FC, y=neglog10q)) + geom_point(aes(col=as.character(region), size = medianCount_Niche_DDLS)) + geom_text_repel(aes(label=labelid), force = 3) + theme_classic() + theme(text = element_text(size=24)))
dev.off()                                                                                                                                                                                
